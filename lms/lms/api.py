...
@frappe.whitelist()
def upsert_chapter(
	title: str, course: str, is_scorm_package: bool, scorm_package: dict = None, name: str = None, parent_chapter: str = None, description: str = None
):
	if not can_modify_course(course):
		frappe.throw(_("You do not have permission to modify this chapter."), frappe.PermissionError)

	values = frappe._dict({"title": title, "course": course, "is_scorm_package": is_scorm_package, "parent_chapter": parent_chapter or "", "description": description or ""})
...
@frappe.whitelist()
def revoke_chapter_access(member: str, course: str, chapter: str):
	"""Revoke a user's access to a specific chapter. Only moderators and course instructors can call this."""
	if not can_modify_course(course):
...
@frappe.whitelist()
def get_chapter_enrollments(course: str, chapter: str):
	"""Return list of members with explicit access to a chapter. Only moderators and instructors."""
	if not can_modify_course(course):
