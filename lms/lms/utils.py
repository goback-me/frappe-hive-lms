...
def get_lesson(course: str, chapter: int, lesson: int) -> dict:
	...
	course_info = frappe.db.get_value(
		"LMS Course",
		course,
		["title", "paid_certificate", "disable_self_learning", "calendly_link"],
		as_dict=1,
	)
	...
	if not lesson_details.include_in_preview and not effective_membership and not can_modify_course(course) and not chapter_access:
		return {
			...
			"calendly_link": course_info.calendly_link,
			"chapter_name": chapter_name,
		}
...
def has_chapter_access(chapter: str, member: str = None) -> bool:
	"""Returns True if the member has been granted explicit access to this chapter."""
	if not member:
		member = frappe.session.user
	if member == "Guest":
		return False
